...
void Renderer2::reload_meshInfos(Scene* scene) {
...
void Renderer2::reload_triangles(Scene *scene) {
    // update triangles
...
void Renderer2::reload_sceneBVH(Scene *scene) {
...
void Renderer2::reload_scene(Scene *scene) {
    reload_meshInfos(scene);
    reload_triangles(scene);
    reload_sceneBVH(scene);
}

void Renderer2::reload_sceneinfos(Scene *scene) {
    reload_meshInfos(scene);
    reload_sceneBVH(scene);
}
...
//    glBindVertexArray(0);
    ...
    // Run compute shader
    glDispatchCompute((SCREEN_H + 15) / 16,
                      (SCREEN_W + 15) / 16,
                      1); // TODO: separate spp
...
Renderer2::Renderer2(const string &shaderPath): ComputeRenderPass(shaderPath) {}
